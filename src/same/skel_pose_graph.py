...
class SkelPoseGraph(torch_geometric.data.Data):
    ...
    def __init__(self, skel_data, pose_data):
        ...
        if skel_data is not None:
            ...
            self.njoints = self.lo.shape[0]

            if hasattr(skel_data, "sym_idx"):
                self.sym_idx = skel_data.sym_idx
    ...
    def ms_from_key(self, key):
        m = self.ms_dict[key + "_m"].to(device=self.lo.device)
        s = self.ms_dict[key + "_s"].to(device=self.lo.device)
        return m, s
    
    def saus_ms_from_key(self, key):
        m = self.saus_ms_dict[key + "_m"].to(device=self.lo.device)
        s = self.saus_ms_dict[key + "_s"].to(device=self.lo.device)
        return m, s
